...
const FeedbackDisplay: React.FC<FeedbackDisplayProps> = ({ feedback, stats }) => {
  // If no feedback or empty message, show default
  const message = feedback.message || "Position yourself in view of the camera";
  
  // Set color based on feedback type
  const messageClass = 
    feedback.type === FeedbackType.WARNING ? 'text-yellow-500 dark:text-yellow-400' :
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-4 gap-2 text-center">
          ...
          <div className="bg-muted p-2 rounded-md">
            ...
            <div className="font-semibold text-yellow-500">{stats.badReps}</div>
          </div>
          <div className="bg-muted p-2 rounded-md">
