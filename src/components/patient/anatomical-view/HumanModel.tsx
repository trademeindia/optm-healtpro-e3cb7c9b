...
const HumanModel: React.FC<HumanModelProps> = ({
  ...
  return <group>
      <Html transform position={[0, 0, 0]} rotation={[0, 0, 0]} center sprite scale={[1.25, 1.25, 1.25]} // Slightly adjusted scale for better fit
    distanceFactor={10}>
        
      </Html>
      
      {hotspots.map(hotspot => <Hotspot key={hotspot.id} position={hotspot.position} color={hotspot.color} size={hotspot.size} label={hotspot.label} description={hotspot.description} severity={hotspot.severity} onClick={() => onHotspotClick(hotspot.id)} />)}
    </group>;
};
