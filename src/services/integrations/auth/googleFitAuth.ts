...
// Auth-related types
export interface GoogleFitAuthCredentials {
  accessToken: string;
  refreshToken: string;
  expiresAt: number;
  userId: string;
...
export class GoogleFitAuth {
  ...
  private readonly STORAGE_KEY = 'googleFit_auth';
  ...
  public storeCredentials(accessToken: string, refreshToken: string, expiresIn: number, userId: string): void {
